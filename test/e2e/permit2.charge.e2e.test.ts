...
describe('e2e Permit2', async () => {
  ...
  it('decode and encode makes successful payment', async () => {
    const jsonCredential = await rawFetchAndMakeDecodedCredential<defaults.Permit2Payload>(
      clientMppx,
      NO_SPLIT_FETCH_ENDPOINT,
    );
  ...
  it('Fails with tampered permitted token', async () => {
    ...
    jsonCredential.payload.permit.permitted[0]!.token =
  ...
  it('Fails with tampered transfer requestedAmount', async () => {
    ...
    const falseValue =
      BigInt(jsonCredential.payload.transferDetails[0].requestedAmount) / BigInt(2);
  ...
  it('Fails with tampered split amount', async () => {
    ...
    const falseValue =
      BigInt(jsonCredential.payload.transferDetails[1].requestedAmount) / BigInt(2);
  ...
  it('Fails with witness challengeHash from a different challenge', async () => {
    const donor = await rawFetchAndMakeDecodedCredential<defaults.Permit2Payload>(
      clientMppx,
      NO_SPLIT_FETCH_ENDPOINT,
    );
    const jsonCredential = await rawFetchAndMakeDecodedCredential<defaults.Permit2Payload>(
  ...
  it('Fails on replay of an already-consumed credential', async () => {
    ...
    const first = await encodeAndSendCredential(
      clientMppx,
      jsonCredential,
      NO_SPLIT_FETCH_ENDPOINT,
    );
    ...
    const second = await encodeAndSendCredential(
      clientMppx,
      jsonCredential,
      NO_SPLIT_FETCH_ENDPOINT,
    );
