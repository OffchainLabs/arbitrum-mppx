...
import { encodeAbiParameters, keccak256, pad, parseEther, toHex } from 'viem';
import type { Account, Address, Hash, Hex, TestClient } from 'viem';
import { privateKeyToAccount } from 'viem/accounts';
import { setBalance, setStorageAt } from 'viem/actions';
...
import { USDC_ALLOWED_SLOT, USDC_BALANCES_SLOT } from './default.e2e.js';
...
export async function seedUsdc(
  ...
  const slot = keccak256(
    encodeAbiParameters([{ type: 'address' }, { type: 'uint256' }], [holder, USDC_BALANCES_SLOT]),
  ) as Hash;
...
export async function seedUsdcAllowance(
  ...
  const slot = keccak256(
    encodeAbiParameters([{ type: 'address' }, { type: 'bytes32' }], [spender, innerSlot]),
  ) as Hash;
...
export async function rawFetchAndMakeDecodedCredential<T>(
  clientMppx: MppxClient.Mppx,
...
export async function encodeAndSendCredential(
  clientMppx: MppxClient.Mppx,
  jsonCredential: JSON,
  fetchEndpoint: string,
) {
